...
export const Card: React.FC<CardProps> = ({
  ...
  const cardClasses = [
    'relative rounded-lg overflow-hidden shadow-md transition-all duration-200 cursor-pointer',
    sizeClasses[size],
  ...
  const handlePointerDown = (e: React.PointerEvent) => {
    try {
      ...
      console.debug('Card.pointerDown', { card, isPlayable, classList: target.className });
    } catch (err) {
  ...
  const getCardTitle = () => {
    ...
    switch (card.value) {
      case 'Skip': title += 'Saltar'; break;
      case 'Reverse': title += 'Reversa'; break;
      case 'DrawTwo': title += '+2'; break;
      default: title += card.value;
    }
